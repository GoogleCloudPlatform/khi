...
- Material symbols MSDF texture and its atlas config JSON
  ...
  - The index.js reads `icons.json` to know which icons are used in `pkg/model/enum` and generates the material symbols MSDF texture with only the used icons. This `icons.json` is generated from the backend code generation.
