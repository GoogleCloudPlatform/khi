...
export class TimelineStore {
  ...
  private timelineChildrenIds: Record<number, number[]> = {};
  private timelineRevisionIds: Undefinable<Uint32Array>[] = [];
  private timelineEventIds: Undefinable<Uint32Array>[] = [];
  private readonly timelineIdToIndex: Undefinable<number>[] = [];
  ...
  private readonly revisionBodies: Undefinable<InternedStruct>[] = [];
  private readonly revisionIdToIndex: Undefinable<number>[] = [];

  // Event metadata
  private eventIds = new Uint32Array(0);
  private eventLogIds = new Uint32Array(0);
  private readonly eventIdToIndex: Undefinable<number>[] = [];
  ...
  public initialize(
    ...
    for (const t of timelines) {
      ...
      this.timelineRevisionIds[t.id] = new Uint32Array(t.revisionIds);
      this.timelineEventIds[t.id] = new Uint32Array(t.eventIds);

      let children = this.timelineChildrenIds[t.parentTimelineId];
      if (!children) {
        children = [];
        this.timelineChildrenIds[t.parentTimelineId] = children;
      }
      children.push(t.id);
    ...
    for (const r of revisions) {
      ...
      if (r.body !== undefined) {
        this.revisionBodies[r.id] = r.body;
      }
  ...
  public getTimeline(id: number): ReadonlyDomainElement<Timeline> {
    if (this.timelineIdToIndex[id] === undefined) {
      throw new Error(`Timeline ID ${id} not found`);
    }
    return new Timeline(id, this);
  }

  public getAllTimelines(): readonly ReadonlyDomainElement<Timeline>[] {
    const timelines: ReadonlyDomainElement<Timeline>[] = [];
    for (let i = 0; i < this.timelineIds.length; i++) {
      timelines.push(new Timeline(this.timelineIds[i], this));
    }
    return timelines;
  }
  ...
  public _getRevisionsForTimeline(
    ...
    const revIds = this.timelineRevisionIds[id];
    if (!revIds) {
      return [];
    }
    ...
    for (let i = 0; i < revIds.length; i++) {
      revisions.push(new Revision(revIds[i], id, this));
    }
    return revisions;
  }
  ...
  public _getEventsForTimeline(id: number): ReadonlyDomainElement<Event[]> {
    const eventIds = this.timelineEventIds[id];
    if (!eventIds) {
      return [];
    }
  ...
  public _getChildIdsForTimeline(id: number): readonly number[] {
    return this.timelineChildrenIds[id] ?? [];
  }
  ...
  public _decodeRevisionBody(
    ...
    const struct = this.revisionBodies[id];
