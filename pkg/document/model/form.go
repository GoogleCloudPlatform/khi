...
func GetFormDocumentModel(taskServer *inspection.InspectionTaskServer) (*FormDocumentModel, error) {
	...
	forms := task.Subset(taskServer.RootTaskSet, task.WhereLabelIsEnabled(label.TaskLabelKeyIsFormTask, false))
...
func getFeaturesRequestingFormTask(taskServer *inspection.InspectionTaskServer, formTask task.UntypedTask) ([]task.UntypedTask, error) {
	...
	features := task.Subset(taskServer.RootTaskSet, task.WhereLabelIsEnabled(inspection_task.LabelKeyInspectionFeatureFlag, false))
