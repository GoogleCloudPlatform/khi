...
func GetInspectionTypeDocumentModel(taskServer *inspection.InspectionTaskServer) InspectionTypeDocumentModel {
	...
	for _, inspectionType := range inspectionTypes {
		...
		tasksInInspectionType := task.Subset(taskServer.RootTaskSet, task.WhereLabelContainsElement(inspection_task.LabelKeyInspectionTypes, inspectionType.Id))
		featureTasks := task.Subset(tasksInInspectionType, task.WhereLabelIsEnabled(inspection_task.LabelKeyInspectionFeatureFlag)).GetAll()
