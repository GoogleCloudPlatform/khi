...
func Register(manager *recorder.RecorderTaskManager) error {
	manager.AddRecorder("binding", []string{}, func(ctx context.Context, resourcePath string, l *types.AuditLogParserInput, prevState any, cs *history.ChangeSet, builder *history.Builder, vs *task.VariableSet) (any, error) {
		return nil, recordChangeSetForLog(ctx, resourcePath, l, cs)
	}, recorder.SubresourceLogGroupFilter("binding"), recorder.AnyLogFilter())
	return nil
}

func recordChangeSetForLog(ctx context.Context, resourcePath string, log *types.AuditLogParserInput, cs *history.ChangeSet) error {
