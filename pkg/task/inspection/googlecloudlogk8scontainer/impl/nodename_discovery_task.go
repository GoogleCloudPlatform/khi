...
var NodeNameDiscoveryTask = inspectiontaskbase.NewInspectionTask(
	...
	func(ctx context.Context, taskMode inspectioncore.InspectionTaskModeType) ([]string, error) {
		...
		logs := coretask.GetTaskResult(ctx, googlecloudlogk8scontainer_contract.ListLogEntriesTaskID.Ref())
		for _, l := range logs {
			fs, err := googlecloudlogk8scontainer_contract.ExtractGCPContainerLogNodeNameLabel(l.NodeReader)
