...
func recordChangeSetForLog(ctx context.Context, resourcePath string, l *commonlogk8saudit_contract.AuditLogParserInput, prevStatus *model.K8sResourceContainingStatus, cs *history.ChangeSet, builder *history.Builder) (*model.K8sResourceContainingStatus, error) {
	...
	isDeletionRequest := deletionStatus == commonlogk8saudit_impl.DeletionStatusDeleted
