...
type mockGKEClusterListFetcher struct {
...
var _ googlecloudclustergke_contract.ClusterListFetcher = (*mockGKEClusterListFetcher)(nil)

func TestAutocompleteGKEClusterNamesTask(t *testing.T) {
	testCase := []struct {
		desc        string
		...
		},
		{
			...
			want: []*googlecloudk8scommon_contract.AutocompleteClusterNameList{
				{
					ClusterNames: []string{},
					Error:        "Failed to list GKE cluster names: test error",
				},
			},
		},
	}
	...
	for _, tc := range testCase {
		t.Run(tc.desc, func(t *testing.T) {
			...
			mockClusterListFetcherInput := tasktest.NewTaskDependencyValuePair[googlecloudclustergke_contract.ClusterListFetcher](googlecloudclustergke_contract.ClusterListFetcherTaskID.Ref(), &mockGKEClusterListFetcher{
			...
			for i := 0; i < len(tc.projectIDs); i++ {
				...
				if diff := cmp.Diff(tc.want[i], result); diff != "" {
					t.Errorf("result of AutocompleteGKEClusterNamesTask mismatch (-want +got):\n%s", diff)
