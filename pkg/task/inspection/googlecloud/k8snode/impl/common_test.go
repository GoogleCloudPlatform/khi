...
func TestK8sNodeLogIngester_ProcessLog(t *testing.T) {
	...
	}{
		{
			...
			assert: func(t *testing.T, cs *khifilev6.LogChangeSet) {
				testchangeset.AssertLog(t, cs).
					...
					HasLogType(k8snode.LogTypeNode)
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			var podIDFinder patternfinder.PatternFinder[*k8snode.PodSandboxIDInfo] = patternfinder.NewNaivePatternFinder[*k8snode.PodSandboxIDInfo]()
			if tc.inputPodIDInfo != nil {
				naiveFinder := patternfinder.NewNaivePatternFinder[*k8snode.PodSandboxIDInfo]()
