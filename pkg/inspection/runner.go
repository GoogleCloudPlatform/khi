...
func (i *InspectionTaskRunner) SetInspectionType(inspectionType string) error {
	...
	i.availableTasks = task.Subset(i.inspectionServer.RootTaskSet, task.WhereLabelContainsElement(inspection_task.LabelKeyInspectionTypes, inspectionType))
	defaultFeatures := task.Subset(i.availableTasks, task.WhereLabelIsEnabled(inspection_task.LabelKeyInspectionDefaultFeatureFlag))
	i.requiredTasks = task.Subset(i.availableTasks, task.WhereLabelIsEnabled(inspection_task.LabelKeyInspectionRequiredFlag))
...
func (i *InspectionTaskRunner) FeatureList() ([]FeatureListItem, error) {
	...
	featureSet := task.Subset(i.availableTasks, task.WhereLabelIsEnabled(inspection_task.LabelKeyInspectionFeatureFlag))
...
func (i *InspectionTaskRunner) addCommonMetadata(ctx context.Context, writableMetadata *typedmap.TypedMap, initHeader *header.Header, taskGraph *task.TaskSet) {
	...
	progressMeta.SetTotalTaskCount(len(task.Subset(taskGraph, task.WhereLabelIsEnabled(inspection_task.LabelKeyProgressReportable)).GetAll()))
