...
読み込みが終了したら、"Open"ボタンをクリックしてビジュアリゼーションを表示します。さまざまなビューがありますが、特にタイムラインビューを確認してください。Nginxデプロイメントの作成、スケール、削除に関連するイベントが表示され、クラスターの状態が時間の経過とともにどのように変化したかを確認できるはずです。
